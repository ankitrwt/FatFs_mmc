...
#else
#define GET_FATTIME()	((DWORD)(2016 - 1980) << 25 | (DWORD)7 << 21 | (DWORD)30 << 16)//get_fattime()
#endif
...
{
	uint8_t time_cntr,Hour_Temp;
	DWORD Date_Time;
	RTC_TimeTypeDef F_time;
  RTC_DateTypeDef F_date;

		time_cntr = 0;
		while(time_cntr++ < 10){
			while(HAL_RTC_GetDate(&hrtc, &F_date,RTC_FORMAT_BIN) != HAL_OK);						 //7th line: Date & Time
			while(HAL_RTC_GetTime(&hrtc, &F_time,RTC_FORMAT_BIN) != HAL_OK);
		}
		...
		{
			if(!F_time.TimeFormat) // AM
			{
				if(F_time.Hours == 12)
					Hour_Temp = 0;
				else
					Hour_Temp = F_time.Hours;
			}
			else // PM
			{
				if(F_time.Hours == 12)
					...
					Hour_Temp = 12 + F_time.Hours;
			}
		}

//		Hour_Temp = 20;
		Date = Date_Time  = ((DWORD)((2000 + F_date.Year) - 1980) << 25 | (DWORD)F_date.Month << 21 | (DWORD)F_date.Date << 16
		| (DWORD)Hour_Temp << 11 | (DWORD)F_time.Minutes << 5 | (DWORD)(F_time.Seconds>>1));
...
ch = 29;
	...
	bsect = 0;
	ch = fmt = check_fs(fs, bsect);					/* Load sector 0 and check if it is an FAT boot sector as SFD */
	ch = 61;
	if (fmt == 1 || (!fmt && (LD2PT(vol)))) {	/* Not an FAT boot sector or forced partition number */
		ch = 62;
		for (i = 0; i < 4; i++) {	ch = 36;		/* Get partition offset */
...
AB3 =5;
	AB1 = res = find_volume(&fs, &path, 0);	/* Force mounted the volume */
AB3 =6;
	LEAVE_FF(fs, res);
}
...
abc++;
	...
	if (res == FR_OK) {
		...
		if (mode & (FA_CREATE_ALWAYS | FA_OPEN_ALWAYS | FA_CREATE_NEW)) {
			abc = 98;
			if (res != FR_OK) {		abc = 133;			/* No file, create new */
				...
				dir = dj.dir;					/* New entry */abc = 157;
			}
			else {			abc = 134;					/* Any object is already existing */
			...
			if (res == FR_OK && (mode & FA_CREATE_ALWAYS)) {	/* Truncate it if overwrite mode */abc = 136;
				dw = GET_FATTIME();
		...
		else {	/* Open an existing file */abc = 99;
		...
		if (res == FR_OK) {		abc = 126;			/* Follow succeeded */
			dir = dj.dir;
