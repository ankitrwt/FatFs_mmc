...
unsigned char SD_init(void)
{
 ...
 for(i=0;i<10;i++)
      SPI_Send_8Bits(0xff);   //80 clock pulses spent before sending the first command
...
SPI_Send_8Bits(cmd | 0x40); //send command, first two bits always '01'
SPI_Send_8Bits(arg>>24);
SPI_Send_8Bits(arg>>16);
SPI_Send_8Bits(arg>>8);
SPI_Send_8Bits(arg);

if(cmd == SEND_IF_COND)	 //it is compulsory to send correct CRC for CMD8 (CRC=0x87) & CMD0 (CRC=0x95)
  SPI_Send_8Bits(0x87);    //for remaining commands, CRC is ignored in SPI mode
else 
  SPI_Send_8Bits(0x95);
...
{
  status = SPI_Read_8Bits() & 0x40;     //first byte of the OCR register (bit 31:24)
  if(status == 0x40) SDHC_flag = 1;  //we need it to verify SDHC card
  else SDHC_flag = 0;

  SPI_Read_8Bits(); //remaining 3 bytes of the OCR register are ignored here
  SPI_Read_8Bits(); //one can use these bytes to check power supply limits of SD
  SPI_Read_8Bits();
}
