...
while(1)
{
		...
		if((dir->name[0] != DELETED) && (dir->attrib != ATTR_LONG_NAME))
        {
			  ...
			  else    //when flag = DELETE
			  {
			     ...
			     while(1)  
			     {
					...
					if(nextCluster > 0x0ffffff6) 
					   {LCD_string("File deleted!");return 0;}
          ...
          else  //when flag = GET_LIST
		  {
		     ...
		     if((dir->attrib != 0x10) && (dir->attrib != 0x08))
			 {
			   ...
			   LCD_string((dir->attrib == 0x10)? "DIR" : "ROOT");
		  }
       }
     }
   }
   ...
   if(cluster > 0x0ffffff6)
   	 return 0;
   if(cluster == 0) 
   {LCD_string("Err getin clustr");  return 0;}
 }
return 0;
}
...
while(1)
{
  ...
  cluster = getSetNextCluster (cluster, GET, 0);
  if(cluster == 0) {LCD_string("Err getin clustr"); return 0;}
}
return 0;
}
...
if(j>8) {LCD_string("Invalid fileName"); return 1;}
...
if(appendFile)  //executes this loop if file is to be appended
{
  ...
  dir->writeTime = 12;//timeFAT;  //setting new time of last write, obtained from RTC
  dir->writeDate = 12;//dateFAT;  //setting new date of last write, obtained from RTC
...
while(1)
{
   ...
   for(sector = 0; sector < sectorPerCluster; sector++)
   {
     ...
     for(i=0; i<bytesPerSector; i+=32)
     {
        ...
        if((dir->name[0] == EMPTY) || (dir->name[0] == DELETED))  //looking for an empty slot to enter file info
		{
		  ...
		  dir->createTime = 12;//timeFAT; 	//setting time of file creation, obtained from RTC
		  dir->createDate = 12;//dateFAT; 	//setting date of file creation, obtained from RTC
		  dir->lastAccessDate = 0;   	//date of last access ignored
		  dir->writeTime = 12;//timeFAT;  	//setting new time of last write, obtained from RTC
		  dir->writeDate = 12;//dateFAT;  	//setting new date of last write, obtained from RTC
   ...
   if(cluster > 0x0ffffff6)
   {
      ...
      {	
	    LCD_string("End Clustr Chain");
	    return;
      }
   }
   if(cluster == 0) {LCD_string("Err getin clustr"); return;}
